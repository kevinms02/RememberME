...
void main() async {
  ...
  await Supabase.initialize(
    url: 'https://dgtwlveibsmobifgcsan.supabase.co',
    anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRndHdsdmVpYnNtb2JpZmdjc2FuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA0MDg1NzIsImV4cCI6MjA2NTk4NDU3Mn0.1u5oceSFwdJT5ZR1KyjGi61YQOBRsm668GZFdBcr_7M',
  );
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (response.user != null) {
        ...
        setState(() {
          _loginError = 'Login gagal: Email/password salah';
        });
      }
    } on AuthException catch (e) {
      setState(() {
        _loginError = 'Login gagal: ${e.message}';
      });
    } catch (e) {
      setState(() {
...
class _MemoriesVaultPageState extends State<MemoriesVaultPage> {
  ...
  Future<void> _fetchMemories() async {
    ...
    try {
      ...
      _memories = (data as List).map((m) => Memory(
        id: m['id'].toString(),
        title: m['title'] ?? '',
...
class _CreateMemoriesPageState extends State<CreateMemoriesPage> {
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              Row(
                children: [
                  ..._mediaPaths.take(3).map((url) => Stack(
  ...
  // Fungsi upload ke Supabase Storage (perbaiki: hapus storagePath yang tidak terpakai)
  ...
  Future<void> _saveMemory(bool isEdit) async {
    ...
    if (isEdit) {
      ...
      final res = await Supabase.instance.client.from('memories').insert({
        'userId': userId,
        'title': _titleController.text,
        'photos': _mediaPaths,
        'date': _dateController.text,
      ...
      memoryId = res['id'].toString();
    }
    setState(() => _loading = false);
    Navigator.pop(context, memoryId); // return memoryId
  }

  // Tulis hanya memoryId ke NFC
  Future<void> _writeToNfc() async {
    String? memoryId;
    if (widget.memory != null) {
      memoryId = widget.memory!.id;
    } else {
      // Simpan dulu jika belum ada
      setState(() => _loading = true);
      final userId = AppState.userId;
      if (userId == null) return;
      final res = await Supabase.instance.client.from('memories').insert({
    ...
    final url = 'https://rememberme.app/memory?id=$memoryId';
    try {
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      final user = await Supabase.instance.client.from('users').select().eq('id', userId).single();
      setState(() {
        AppState.currentUser = User(
          name: user['name'],
          email: user['email'],
